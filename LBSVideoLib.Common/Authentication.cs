...
namespace LBFVideoLib.Common
{
    public class Authentication
    {
        ...
        static string _clientInfoFilePathEncrypted = Path.Combine(ConfigHelper.ClientDistributionTargetRootPath, "clientInfo.txt");
        // Authenticate Admin
     ...
     adminInfo=    Cryptograph.DecryptObject<AdminInfo>(adminInfoFilePathEncrypted);
        ...
        public static bool AuthenticateClient(string email, string password)
        {
            ...
            {
                authenticated= true;
                clientInfo.LastAccessStartTime = DateTime.UtcNow;
                clientInfo.LastAccessEndTime= DateTime.UtcNow;
                Cryptograph.EncryptObject(clientInfo, _clientInfoFilePathEncrypted);
            }
            else
            {
                authenticated = false;
